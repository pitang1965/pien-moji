...
const Config = () => {
  ...
  return (
    <div className='config'>
      ...
      <Slider parameterTitle={'目の大きさ'} onChange={(_, value) => configContext.setEyeSize(value)} min={0.7} max={1.0} step={0.1} value={configContext.eyeSize} />
      ...
      <Slider parameterTitle={'ほっぺの大きさ'} onChange={(_, value) => configContext.setCheekSize(value)} min={0.7} max={1.3} step={0.1} value={configContext.cheekSize} />
    </div>
  );
};
